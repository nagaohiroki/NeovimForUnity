	...
	internal class Messager : IDisposable
	{
		...
		private void ReceiveMessageCallback(IAsyncResult result)
		{
			try
			{
				...
				var message = DeserializeMessageRaw(UdpSocket.BufferFor(result));
				if (message != null)
				{
					...
					if (IsValidTcpMessage(message, out var port, out var bufferSize))
					{
						...
						{
							var originalMessage = DeserializeMessageRaw(buffer);
		...
		private static Message DeserializeMessageRaw(byte[] buffer)
		{
			var str = Encoding.UTF8.GetString(buffer);
			var json = JsonUtility.FromJson<MessageJson>(str);
			if (json == null)
			{
				Debug.LogError($"Failed to deserialize message: {str}");
				return null;
			}
			if (!Enum.TryParse<MessageType>(json.Type, out var type))
			{
				Debug.LogError($"Failed to parse message type: {json.Type}");
				return null;
			}
			return new Message { Type = type, Value = json.Value };
		}
